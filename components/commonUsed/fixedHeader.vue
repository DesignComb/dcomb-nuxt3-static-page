...
<template>
  <div class="hidden md:flex max-w-content px-12 py-3 justify-end">
        <nuxt-link v-for="(link,index) in headerLink" :key="link.name" :to="link.link" class="decoration-clip-border relative mr-6">
            <div class="decoration-clip-path italic font-bold tracking-wide relative px-8 py-1 hover:bg-comb hover:text-white transition hover:transition-delay-300 duration-500 ease-in-out border-box">
              {{ link.name }}
            </div>
        </nuxt-link>
    </div>
...
<!--          <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 256 256"><path fill="currentColor" d="m120 137l-72 64a12 12 0 1 1-16-18l61.91-55L32 73a12 12 0 1 1 16-18l72 64a12 12 0 0 1 0 18Zm96 43h-96a12 12 0 0 0 0 24h96a12 12 0 0 0 0-24Z"/></svg>-->
    ...
    <div ref="swiperDownDetector" @click="clickOpenMobileHeader"  :class="store.isMobileHeader ? 'op100': 'op0 transition delay-300 duration-200'"
...
<style scoped>
.decoration-clip-path {
    clip-path: polygon(15% 0, 100% 0, 100% 72.5%, 85% 100%, 0 100%, 0 27.5%);
}

.decoration-clip-border:after,
.decoration-clip-border:before {
...
.decoration-clip-border:hover:before{
    top: 55%;
    left: 0%;
    opacity: 0; /* 懸停時將:before和:after元素的透明度設為0 */
}
.decoration-clip-border:hover:after {
    top: -10%;
    left: 100%;
    opacity: 0; /* 懸停時將:before和:after元素的透明度設為0 */
}
